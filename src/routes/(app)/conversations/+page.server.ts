...
import { delete_conversation, favorite_conversation } from "$lib/server/database/conversations";
...
export const actions = {
    ...
    delete: async ({ request, cookies }: { request: Request, cookies: Cookies }) => {
        const data: FormData = await request.formData();
        let conversation: Conversation = JSON.parse(data.get("conversation")!.toString());
        ...
        await delete_conversation(current_user, conversation.uuid);
    },
}
